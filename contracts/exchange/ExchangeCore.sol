...
/*
  ...
  - Buy-side and sell-side orders each provide _calldata (bytes) - for a sell-side order, the state transition for sale, for a buy-side order, the state transition to be bought.
    Along with the _calldata, orders provide `replacementPattern`: a bytemask indicating which bytes of the _calldata can be changed (e.g. NFT destination address).
    When a buy-side and sell-side order are matched, the desired _calldatas are unified, masked with the bytemasks, and checked for agreement.
  ...
  - Orders (of either side) can optionally specify a static (no state modification) callback function, which receives configurable data along with the actual _calldata as a parameter.
    ...
    Use of the EVM's STATICCALL opcode, added in Ethereum Metropolis, allows the static _calldata to be safely specified separately and thus this kind of matching to happen correctly
...
contract ExchangeCore is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function transferTokens(
        ...
        if (amount > 0) {
            require(tokenTransferProxy.transferFrom(IERC20(token), from, to, amount));
        }
    }
