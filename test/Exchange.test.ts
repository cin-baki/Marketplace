...
describe('Exchange Test Suite', () => {
  ...
  let SipherNFT: MockERC721;
  before(async () => {
    [owner, account1, account2] = await ethers.getSigners();
  });
  beforeEach(async () => {
    // Deploy Sipher NFT
    const SipherNFTFactory: MockERC721__factory = await ethers.getContractFactory('MockERC721');
    SipherNFT = await SipherNFTFactory.deploy();
    await SipherNFT.mint(owner.address, '1');
  ...
  it('should match order', async () => {
    const accounts = await ethers.getSigners();
    SipherNFT.setApprovalForAll(WyvernExchange.address, true);
    const buyCalldata = SipherNFT.interface.encodeFunctionData('transferFrom', [
      constants.AddressZero,
      accounts[1].address,
      '1',
    ]);
    const sellCaldata = SipherNFT.interface.encodeFunctionData('transferFrom', [
      accounts[0].address,
      constants.AddressZero,
      '1',
    ]);
    ...
    buy.taker = constants.AddressZero;
    sell.side = 1;
    buy.target = SipherNFT.address;
    sell.target = SipherNFT.address;
  ...
  describe('Offer and Cancelled Offer', () => {
    it('should cancel order', async () => {
      const accounts = await ethers.getSigners();
      SipherNFT.connect(account1).setApprovalForAll(WyvernExchange.address, true);
  ...
  describe('Listing and Cancelled Listing', () => {
    it('should cancel order', async () => {
      ...
      const sellCalldata = SipherNFT.interface.encodeFunctionData('transferFrom', [
        constants.AddressZero,
        account1.address,
        '1',
      ]);
